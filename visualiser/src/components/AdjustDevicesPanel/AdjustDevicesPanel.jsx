...
export function AdjustDevicesPanel() {
  ...
  // Default hidden on first load; respect persisted preference if available
  const [open, setOpen] = useState(() => {
    try {
      const v = typeof localStorage !== 'undefined' ? localStorage.getItem('__abacws_adjust_open') : null;
      if (v === '1') return true;
      if (v === '0') return false;
    } catch (_) {}
  ...
  return (
    ...
    <button
      ...
      onClick={() => setOpen(o => {
        const next = !o;
        try { if (typeof localStorage !== 'undefined') localStorage.setItem('__abacws_adjust_open', next ? '1' : '0'); } catch(_) {}
        return next;
      })}
    >
      {open ? '⟨' : '⟩'}
